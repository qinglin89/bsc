...
const (
	...
	MpImportingCommitTotal     MpMetricsName = "MP_IMPORTING_COMMIT_TOTAL"
...
var (
	...
	importingCommitTotalTimer      = metrics.NewRegisteredTimer("mp/importing/commit/total", nil)
	importingCommitTotalAllCounter = metrics.NewRegisteredCounter("mp/importing/commit/total/all", nil)
...
func RecordMPMetrics(metricsName MpMetricsName, start time.Time) {
	...
	case MpImportingCommitTotal:
		recordTimer(importingCommitTotalTimer, start)
		increaseCounter(importingCommitTotalAllCounter, start)
